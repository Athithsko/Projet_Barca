...
def load_raw_key_players_data():
    ...
    """Isn't useful anymore because I changed the data set for adding some usefule values
    players_df = players_df.rename(columns={'ï»¿Player': 'Players'})
    players_df['Players'] = players_df['Players'].replace('MartÃ­nez', 'Martinez')"""
    
    players_df = players_df.rename(columns={'Player': 'Players'})
...
def validate_players_data(players_df):
    ...
    numeric_columns = ['Succes_P%', 'Sot%', 'TO%', 'xG', 'PrgC', 'PrgP', 'Pass', 
                       'Shots', 'Take_On', 'Tkl', 'TklW', 'Int', 'Recov', 'Defduel', 'Defduel%']
    ...
    percentage_cols = ['Succes_P%', 'Sot%', 'TO%','Defduel%']
